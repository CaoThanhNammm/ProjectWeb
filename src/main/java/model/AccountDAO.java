...
//				return h.createQuery("SELECT " + "*" + " FROM " + NAME_TABLE + " WHERE " + prefix + "=:name" + " AND "
//						+ PASSWORD + "=:password").bind("name", name2).bind("password", Encrypt.encrypt(password))
...
//			});
	...
	public static boolean hasAccount(final String email, final String phone) {
		Account ac = null;
		try {
			ac = connect.withHandle(h -> {
				return h.createQuery("SELECT " + ID + " FROM " + NAME_TABLE + " WHERE " + EMAIL + "=:email" + " OR " + PHONE + "=:phone")
						.bind("email", email).bind("phone", phone).mapToBean(Account.class).findOne().orElse(null);
			});
		} catch (Exception e) {
			e.printStackTrace();
		}
